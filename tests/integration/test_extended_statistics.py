...
from ext_stat_plugin.extended_statistic.sqlalchemy import ExtendedStatistics, NDISTINCT, MCV, DEPENDENCIES
...
def test_extended_statistics_lifecycle(engine: Engine):
    ...
    table = Table(
        ...
        ExtendedStatistics("orders_city_state_stats", [NDISTINCT, DEPENDENCIES], "city", "state"),
    ...
    try:
        ...
        assert "create_statistics('public', 'orders', 'orders_city_state_stats'" in script1
        assert "create_statistics('public', 'orders', 'public_orders_user_id_city_stats'" in script1
        ...
        with engine.connect() as conn:
            ...
            assert stats["orders_city_state_stats"]["kinds"] == {"NDISTINCT", "DEPENDENCIES"}
            ...
            assert stats["public_orders_user_id_city_stats"]["columns"] == ["user_id", "city"]
        ...
        assert "create_statistics('public', 'orders', 'orders_user_state_stats'" in script2
...
def test_extended_statistics_custom_schema(engine: Engine):
    ...
    Table(
        ...
        ExtendedStatistics("test_items_cat_subcat_stats", [NDISTINCT, MCV], "category", "subcategory"),
    )

    runner = AlembicRunner(engine, schema_metadata, schema="test_schema")
    try:
        script = runner.autogenerate("create_test_items_stats")
        assert "create_statistics('test_schema', 'items', 'test_items_cat_subcat_stats'" in script
        ...
        with engine.connect() as conn:
            ...
            assert stats["test_items_cat_subcat_stats"]["columns"] == ["category", "subcategory"]
